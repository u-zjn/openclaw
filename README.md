...
| # | 决策 | 优先级 | 影响 |
|:-:|:-----|:------|:------|
| 21 | **不使用 API 加签鉴权** — 知乎 API 使用 Cookie 而非标准的 OAuth2 Token，Cookie 有失效风险 | 🟡 P2 |中|
| 22 | **不使用数据库** — 所有 Agent 协作通过文件系统 `collab/` 目录完成，无事务性保障 | 🟢 P3 |低|
| 23 | **不使用消息队列** — Agent 之间直接文件读写，非异步消息驱动 | 🟢 P3 |低|
